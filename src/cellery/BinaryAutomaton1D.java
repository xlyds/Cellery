...
public class BinaryAutomaton1D extends Automaton1D{
	...
	/**
	 * Constructs a 2-state {@link Automaton1D} from an initial state given by a {@link CellArray1D} and a {@link Rule}.
	...
	public void iterate() {
		...
		for (int i = 0; i < this.length; i++) {
			...
			if ( (i == 0 || i == length - 1) && this.getRule().ignoreBoundary == true) {
				if (thisCell.isAlive())
					buffer.reviveCell(i);
				else
					buffer.killCell(i);
