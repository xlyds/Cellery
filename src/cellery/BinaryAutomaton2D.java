...
public class BinaryAutomaton2D extends Automaton2D {
	...
	/**
	 * Constructs a 2-state {@link Automaton2D} from an initial state given by a {@link CellArray2D} and a {@link Rule}.
	 * @param cells A {@link CellArray2D} that provides the initial state of this 
	 * @param rule The {@link Rule} that governs the evolution of this.
	 */
	...
	public void iterate() {
		...
		for (int i = 0; i < this.length; i++) {
			for (int j = 0; j < this.width; j++) {
				...
				if ( (i == 0 || j == 0 || i == length - 1 || j == width - 1) && this.getRule().ignoreBoundary == true) {
					if (thisCell.isAlive())
